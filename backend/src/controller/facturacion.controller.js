...
async function generateInvoiceNumber() {
  ...
  const formattedInvoiceNumber = `GA-${nextInvoiceNumber}`;
...
const getSalesCountByDate = async (req, res, next) => {
  try {
    ...
    // Consultar la base de datos para contar las ventas en el día especificado
    const salesCount = await InvoiceFactura.count({
      where: {
        createdAt: {
