...
const createProduct = async (req, res, next) => {
  try {
    const { name, price, barcode,description, quantity, costo } = req.body;
...
const deleteProductP = async (req, res, next) => {
  ...
  const obtenerCantidadTotalMercancia = async (req, res, next) => {
    try {
      // Consultar la base de datos para obtener la cantidad total de mercancía en inventario
      // Por ejemplo, puedes sumar las cantidades de todos los productos en la tabla de inventario
      const cantidadTotalMercancia = await Product.sum('quantity');
  
      // Consultar la base de datos para obtener estadísticas de productos
      // Por ejemplo, puedes obtener la cantidad total de productos, productos más vendidos, etc.
      const estadisticasProductos = {
        cantidadTotalProductos: await Product.count(),
        // Otras estadísticas aquí...
      };
  
      res.status(200).json({ cantidadTotalMercancia, estadisticasProductos });
    } catch (error) {
      next(error);
    }
  };
  ...
  const obtenerEstadisticasCierreMes = async (req, res, next) => {
    try {
      // Obtener la fecha actual para el cierre de mes
      const fechaActual = new Date();
      const mesActual = fechaActual.getMonth() + 1;
      const anioActual = fechaActual.getFullYear();
  
      // Obtener el primer día del mes actual
      const primerDiaMesActual = new Date(anioActual, mesActual - 1, 1);
  
      // Obtener el último día del mes actual
      const ultimoDiaMesActual = new Date(anioActual, mesActual, 0);
  
      // Consultar la base de datos para obtener las ventas del mes actual
      const ventasMesActual = await InvoiceFactura.findAll({
        where: {
          createdAt: {
      ...
      // Consultar la base de datos para obtener la cantidad total de productos en inventario
      const cantidadTotalProductos = await Product.sum('quantity');
  
      // Obtener la cantidad total de productos vendidos en el mes actual
      const cantidadProductosVendidos = ventasMesActual.reduce((total, venta) => {
        return total + venta.productos.reduce((subtotal, producto) => {
          return subtotal + producto.pivot.quantity;
        }, 0);
      }, 0);
  
      // Otras estadísticas que quieras obtener...
      ...
      console.error(error);
      res.status(500).json({ message: 'Ocurrió un error al obtener las estadísticas de cierre de mes' });
    }
  };
...
const obtenerInventarioInicialYFinal = async (req, res, next) => {
  try {
    ...
    const productosEnInventario = await Product.findAll({
      attributes: ['id', 'name', 'quantity', 'costo', 'price'], // Obtener solo los campos 'id', 'name' y 'quantity'
    });
    ...
    //   attributes: [
    //     'productoFactura',
    //     [sequelize.fn('sum', sequelize.col('amount')), 'totalVentasPorProducto'],
    //   ],
    //   where: {
    //     createdAt: {
    //       $between: [fechaInicial, fechaFinal],
    //     },
    //   },
    //   group: ['productoFactura'],
    // });

    // Crear un objeto para almacenar la información del inventario inicial y final
    const inventarioInicialYFinal = {
      fechaInicial,
      fechaFinal,
    ...
    console.error(error);
    res.status(500).json({ message: 'Ocurrió un error al obtener el inventario inicial y final' });
    next(error);
  }
};
...
module.exports = {
  ...
  obtenerCantidadTotalMercancia,
  obtenerInventarioInicialYFinal 
};
