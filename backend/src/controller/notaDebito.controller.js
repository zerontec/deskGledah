...
const crearNotaDebito = async (numeroNota, fecha, monto, clienteData) => {
  try {
    ...
    console.log("Nota de Debito creada exitosamente");
  } catch (error) {
...
const editNota = async (req, res, next) => {
  ...
  try {
    const nots = await NotaDebito.findByPk(id);
    if (nots) {
...
const obtenerUnaNota = async (req, res, next) => {
  try {
    ...
    const nota = await NotaDebito.findByPk(id);
    if (id) {
