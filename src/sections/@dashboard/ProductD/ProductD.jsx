...
import { deleteMultiplyProductsD, deleteProductD, getAllProductD, updateProductD } from '../../../redux/modules/productosD';
...
const ProductD = () => {
	...
	const handleDeleteMultipleClick = () => {
	  ...
	  const productos = useSelector((state) => state.productd);
	  ...
	  function deleteHandler(items) {
		Swal.fire({
		  ...
		  if (result.isConfirmed) {
			dispatch(deleteProductD(items.id));
	  ...
	  const handleSubmit = (e) => {
		if (selectedProductEdit.name && selectedProductEdit.description && selectedProductEdit.price) {
		  ...
		  dispatch(updateProductD(selectedProductId, data));
			...
			>
			  ...
			  {selectedProduct && (
				...
				<strong>Codigo:</strong> {selectedProduct.barcode}
				  ...
				  <p>
					<strong>Cantidad Devuelta:</strong> {selectedProduct.cantidadDevuelta}
				  </p>
				  <p>
					<strong>Factura de Venta:</strong> {selectedProduct.invoiceNumber}
				  </p>
				  <p>
				  <strong>Fecha Devolucion:</strong> {fDate(selectedProduct.fechaDevolucion)}
				</p>
				<p>
					<strong>Motivo:</strong> {selectedProduct.description}
				  </p>
			...
			<TableContainer component={Paper}>
			  <Table sx={{ minWidth: 650 }}>
				...
				<TableBody>
				  ...
				  {Array.isArray(productos.productds) && productos.productds.length > 0 ?(
					productos.productds
					...
					/>
						  ...
						  <TableCell align="left"> {items.cantidadDevuelta}</TableCell>
			  ...
			  <TablePagination
				...
				component="div"
				count={productos.productds.length}
