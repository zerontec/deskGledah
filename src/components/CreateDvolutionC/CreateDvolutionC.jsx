...
import { createDevolution } from '../../redux/modules/devolutionC';

import { getAllPurchases } from '../../redux/modules/purchase';
  ...
  //   const [productList, setProductList] = useState([]);
	...
	const handleProductChange = (index, field, value) => {
		setDevolutionData((prevData) => {
		  ...
		  if (field === 'quantity') {
			// Solo convertir a número si el campo es 'quantity'
			newProducts[index][field] = parseInt(value, 10);
		  } else {
			// De lo contrario, mantener el valor como una cadena
	...
	const addProduct = () => {
	  setDevolutionData((prevData) => ({
		...prevData,
		productos: [...prevData.productos , {barcode: '', quantity:0}],
...
// 	  const updateProduct={
// 			...devolutionData,
// 			barcode:devolutionData.productos.barcode,
// 			quantity:parseInt(devolutionData.productos.quantity, 10)

// 	  };

// updateProducts=[ ...devolutionData,updateProduct]


// 	  setDevolutionData(updateProducts)
	};
  ...
  if(validator.isEmpty(devolutionData.purchaseNumber)){

		errors.rif= 'El numero de Compra es Requerido '
...
console.log("devolutionD", devolutionData);
				...
				<FormContainer>
				  <FieldContainer>
					<TextField
					  ...
					  type="number"
					  id="purchaseNumber"
					  value={devolutionData.purchaseNumber}
					  onChange={handleInputChange}
					/>{' '}
					{validationErrors.purchaseNumber && <span>{validationErrors.purchaseNumber}</span>}
					
					<TextField
					  required
					  label="Numero de Factura Compra "
					  name="invoiceNumber"
					  type="number"
					  id="purchaseNumber"
...
export const CreateDvolutionCStl = styled.div``;

CreateDvolutionC.propTypes = {};

export default CreateDvolutionC;