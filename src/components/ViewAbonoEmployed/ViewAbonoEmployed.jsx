...
const ViewAbonoEmployed = ({loanId}) => {
	...
	const handleVerPagos = async (loanId) => {
	  try {
		const response = await axios.post(`${API_URL_D}api/loan/get-payments`,{loanId}, { headers: authHeader() });
					  ...
					  <TableBody>
						{selectedCuentaPagos.map((pago) => (
						  <TableRow key={pago.id}>
							...
							<TableCell>{pago.sellerId}</TableCell>
...
export const ViewAbonoEmployedStl = styled.div``;

ViewAbonoEmployed.propTypes = {};

export default ViewAbonoEmployed;